...
import { Prescription, CreatePrescriptionRequest, PrescriptionStatus } from '../models/prescription.model';
...
export class PrescriptionService {
  ...
  private selectedPrescriptionSubject = new BehaviorSubject<Prescription | null>(null);
  ...
  getPrescriptionById(id: string): Observable<Prescription> {
    return this.http.get<Prescription>(`${this.apiUrl}/${id}`).pipe(
      tap(prescription => this.selectedPrescriptionSubject.next(prescription))
    );
  }
   ...
   * Update prescription
   */
  updatePrescription(id: string, prescriptionData: Partial<Prescription>): Observable<Prescription> {
    return this.http.patch<Prescription>(`${this.apiUrl}/${id}`, prescriptionData).pipe(
      tap(prescription => {
  ...
  updatePrescriptionStatus(id: string, status: PrescriptionStatus): Observable<Prescription> {
    return this.http.patch<Prescription>(`${this.apiUrl}/${id}/status`, { status }).pipe(
      tap(prescription => {
        if (this.selectedPrescriptionSubject.value?.id === id) {
          this.selectedPrescriptionSubject.next(prescription);
        }
  ...
  getPrescriptionHistory(patientId: string): Observable<Prescription[]> {
    return this.http.get<Prescription[]>(`${this.apiUrl}/patient/${patientId}/history`);
  }
  ...
  renewPrescription(id: string, validUntil?: Date): Observable<Prescription> {
    return this.http.post<Prescription>(`${this.apiUrl}/${id}/renew`, { validUntil });
  }
